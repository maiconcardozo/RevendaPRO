import { SuppliersView } from "@/components/suppliers/SuppliersView";
import { fetchFromApi } from "@/lib/server";
import { requireScreen } from "@/lib/session";
import type { Supplier, SupplierSegment, SupplierSpend } from "@/lib/types";

/**
 * Fornecedores, e quanto já foi para cada um.
 *
 * Cadastro e leitura na mesma tela: quem cadastra o fornecedor é quem quer saber quanto foi
 * para ele. O gasto chega "desde o início"; o período é escolhido na tela.
 */
export default async function SuppliersPage() {
  await requireScreen("suppliers");

  const [suppliers, segments, spending] = await Promise.all([
    fetchFromApi<Supplier[]>("suppliers"),
    fetchFromApi<SupplierSegment[]>("supplier-segments"),
    fetchFromApi<SupplierSpend[]>("suppliers/spending"),
  ]);

  return (
    <SuppliersView
      initialSuppliers={suppliers}
      initialSegments={segments}
      initialSpending={spending}
    />
  );
}
